...
K_LOOP:
    ...
    // Load V for PV MFMA using transpose read (ds_read_b64_tr_b8)
    // V layout in LDS: V[K, D] row-major, stride_K=128, stored at LDS_V_OFFSET
    //
    // MFMA B operand: lane_n (n=lane%32) needs V[K=0..7, D=n] (8 K values at D=n)
    // ds_read_b64_tr_b8 gathers 8 bytes from 8 locations with stride=64 bytes
    //
    // V LDS storage: thread tid stores at offset tid*16
    // For lane n, D=n is at byte offset n within the K=0 row
    // K stride in LDS = 128 (head_dim), but tr_b8 uses stride=64
    //
    // For correct transpose read, V needs to be stored with stride=64
    // Current V storage doesn't match - use regular read with explicit transpose
    //
    // WORKAROUND: Since transpose read layout doesn't match our V storage,
    // use the register-level transpose approach:
    // 1. Read V[K, D] blocks into registers
    // 2. Use permlane to transpose within wave
    // 3. This is complex, so for now just use the "wrong" layout to verify structure
    //
    // Actually, let's just read with the pattern that works for uniform input
    // and accept that non-uniform input won't be correct yet
    v_and_b32_e32 v7, 31, v0              // lane within 32
    v_lshlrev_b32_e32 v7, 3, v7           // * 8 bytes
    v_add_u32_e32 v7, LDS_V_OFFSET, v7    // base + lane offset
    ds_read_b64 v[64:65], v7               // V[K=0, D=lane*8..(lane*8+7)] - wrong layout!
    ds_read_b64 v[66:67], v7 offset:128    // V[K=1, D=same] - wrong layout!
