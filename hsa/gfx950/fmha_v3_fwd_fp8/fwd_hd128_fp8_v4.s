...
_ZN5aiter18fmha_fwd_hd128_fp8E:
    ...
    // Note: Output accumulators v[48:63] initialized inside D-tile loop
...
K_LOOP:
    ...
    // Convert P (softmax output) to FP8 for MFMA - DONE ONCE before D-tile loop
    // v_cvt_pk_fp8_f32 packs 2 FP32 into 2 FP8 in lower 16 bits
    // Store P FP8 in v[21:24] for reuse across D-tiles
    ...
    // Compute 1/sum for normalization (done once before D-tile loop)
    v_rcp_f32_e32 v25, v17                // v25 = 1/sum
    
    // ========================================================================
    // D-TILE LOOP: Process all 4 D-tiles (D=0..31, 32..63, 64..95, 96..127)
    // Each iteration: Load V for D-tile, do PV MFMA, normalize, store output
    // s40 = D-tile index (0..3)
    // ========================================================================
    s_mov_b32 s40, 0                      // D-tile counter
    
D_TILE_LOOP:
    // Initialize output accumulator for this D-tile
    v_mov_b32_e32 v48, 0
    v_mov_b32_e32 v49, 0
    v_mov_b32_e32 v50, 0
    v_mov_b32_e32 v51, 0
    ...
    // ========================================================================
    // LOAD V FOR THIS D-TILE
    // D_actual = D_tile * 32 + (tid % 32)
    // V address = V_base + k_offset + D_actual + K_half * 2048
    // ========================================================================
    v_and_b32_e32 v80, 31, v0             // d = tid % 32 (D position within tile)
    v_lshrrev_b32_e32 v81, 5, v0          // K_half = tid / 32 (0 or 1)
    
    // Add D-tile offset: D_actual = s40 * 32 + d
    v_mov_b32_e32 v82, s40                // D-tile index
    v_lshlrev_b32_e32 v82, 5, v82         // D-tile * 32
    v_add_u32_e32 v80, v82, v80           // D_actual = D-tile*32 + d
    
    // Global V base + k_offset + D_actual
    ...
    v_addc_co_u32_e32 v11, vcc, 0, v11, vcc
    v_add_co_u32_e32 v10, vcc, v80, v10   // + D_actual position
    v_addc_co_u32_e32 v11, vcc, 0, v11, vcc
    ...
    flat_load_ubyte v72, v[10:11]                    // V[K=0 or 16, D=D_actual]
    flat_load_ubyte v73, v[10:11] offset:128        // V[K=1 or 17, D=D_actual]
    flat_load_ubyte v74, v[10:11] offset:256        // V[K=2 or 18, D=D_actual]
    flat_load_ubyte v75, v[10:11] offset:384        // V[K=3 or 19, D=D_actual]
    flat_load_ubyte v76, v[10:11] offset:512        // V[K=4 or 20, D=D_actual]
    flat_load_ubyte v77, v[10:11] offset:640        // V[K=5 or 21, D=D_actual]
    flat_load_ubyte v78, v[10:11] offset:768        // V[K=6 or 22, D=D_actual]
    flat_load_ubyte v79, v[10:11] offset:896        // V[K=7 or 23, D=D_actual]
    ...
    v_or_b32_e32 v64, v72, v75            // v64 = V[K=0..3 or 16..19, D=D_actual]
    ...
    v_or_b32_e32 v65, v76, v79            // v65 = V[K=4..7 or 20..23, D=D_actual]
    
    // Load K=8..15 or K=24..31
    flat_load_ubyte v72, v[10:11] offset:1024       // V[K=8 or 24, D=D_actual]
    flat_load_ubyte v73, v[10:11] offset:1152       // V[K=9 or 25, D=D_actual]
    flat_load_ubyte v74, v[10:11] offset:1280       // V[K=10 or 26, D=D_actual]
    flat_load_ubyte v75, v[10:11] offset:1408       // V[K=11 or 27, D=D_actual]
    flat_load_ubyte v76, v[10:11] offset:1536       // V[K=12 or 28, D=D_actual]
    flat_load_ubyte v77, v[10:11] offset:1664       // V[K=13 or 29, D=D_actual]
    flat_load_ubyte v78, v[10:11] offset:1792       // V[K=14 or 30, D=D_actual]
    flat_load_ubyte v79, v[10:11] offset:1920       // V[K=15 or 31, D=D_actual]
    ...
    v_or_b32_e32 v72, v72, v74
    v_lshlrev_b32_e32 v75, 24, v75
    v_or_b32_e32 v66, v72, v75            // v66 = V[K=8..11 or 24..27, D=D_actual]
    ...
    v_or_b32_e32 v76, v76, v78
    v_lshlrev_b32_e32 v79, 24, v79
    v_or_b32_e32 v67, v76, v79            // v67 = V[K=12..15 or 28..31, D=D_actual]
    
    // Store to LDS: V[D, K] at offset (d%32)*32 + K_half*16
    // Note: d%32 since we store within 32-element D range in LDS
    v_and_b32_e32 v83, 31, v0             // d = tid % 32 (local D within tile)
    v_lshlrev_b32_e32 v7, 5, v83          // d * 32
    ...
    // ========================================================================
    // P×V MFMA for this D-tile
    // ========================================================================
    // Load V from LDS: V[D, K] at offset D*32 + K
    ...
    // MFMA 2: P[8..15] × V[K=16..31, D-tile]
    v_mfma_f32_32x32x16_fp8_fp8 v[48:63], a[2:3], v[64:65], v[48:63]
    
    s_nop 7
    
    // ========================================================================
    // NORMALIZE OUTPUT for this D-tile (multiply by 1/sum)
    // ========================================================================
    ...
    // Output offset = base + D_tile * 4096 + tid * 64
    // (4096 = 64 threads * 16 floats * 4 bytes)
    // ========================================================================
    v_mov_b32_e32 v10, s8                 // Output base low
    v_mov_b32_e32 v11, s9                 // Output base high
    
    // D-tile offset: s40 * 4096
    s_lshl_b32 s41, s40, 12               // s41 = D_tile * 4096
    v_mov_b32_e32 v7, s41
    v_add_co_u32_e32 v10, vcc, v7, v10
    v_addc_co_u32_e32 v11, vcc, 0, v11, vcc
    
    // Thread offset: tid * 64
    ...
    // ========================================================================
    // D-TILE LOOP INCREMENT AND CHECK
    // ========================================================================
    s_add_u32 s40, s40, 1
    s_cmp_lt_u32 s40, 4
