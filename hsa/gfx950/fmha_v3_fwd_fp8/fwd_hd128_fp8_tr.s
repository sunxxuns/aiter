...
K_LOOP:
    ...
    // ========================================================================
    // LOAD V TILE WITH STRIDE_K=32 LAYOUT FOR ds_read_b64_tr_b8
    // ========================================================================
    // For ds_read_b64_tr_b8, stride appears to be 32 (not 64)
    //   V[K, D] must be at LDS offset: K*32 + D
    //   (stride_K=32, stride_D=1)
    //
    // With stride_K=32 and D-tile of 32 columns:
    // LDS region = 32 K rows * 32 bytes/row = 1024 bytes per D-tile
    //
    // Thread tid loads: K_row = tid / 2, D_half = tid % 2
    // Load 16 bytes: V[K_row, D_half*16..(D_half*16+15)]
    // Store at: LDS[K_row * 32 + D_half * 16]
    
    // Compute K row and D offset for this thread  
    v_lshrrev_b32_e32 v80, 1, v0          // K_row = tid / 2 (0..31)
    v_and_b32_e32 v81, 1, v0              // D_half = tid % 2 (0 or 1)
    v_lshlrev_b32_e32 v81, 4, v81         // D_offset = D_half * 16 (0 or 16)
    
    // Global V address: V_base + k_offset + K_row * 128 + D_offset
    v_mov_b32_e32 v10, s14                // V base low
    v_mov_b32_e32 v11, s15                // V base high
    v_mov_b32_e32 v7, s33                 // k_offset (k_tile_idx * 8192)
    v_add_co_u32_e32 v10, vcc, v7, v10
    v_addc_co_u32_e32 v11, vcc, 0, v11, vcc
    v_lshlrev_b32_e32 v7, 7, v80          // K_row * 128 (global stride)
    v_add_co_u32_e32 v10, vcc, v7, v10
    v_addc_co_u32_e32 v11, vcc, 0, v11, vcc
    v_add_co_u32_e32 v10, vcc, v81, v10   // + D_offset
    v_addc_co_u32_e32 v11, vcc, 0, v11, vcc
    
    // Load 16 bytes of V: V[K_row, D_offset..(D_offset+15)]
    flat_load_dwordx4 v[72:75], v[10:11]
    
    s_waitcnt vmcnt(0)
    
    // Store V to LDS with stride_K=32 layout
    // LDS offset = K_row * 32 + D_offset
    v_lshlrev_b32_e32 v7, 5, v80          // K_row * 32 (LDS stride)
    v_add_u32_e32 v7, v81, v7             // + D_offset
    ...
    // Load V using ds_read_b64_tr_b8 (transpose read for FP8)
    // V in LDS: stride_K=32, V[K, D] at offset K*32 + D
    //
    // ds_read_b64_tr_b8 at base reads 8 bytes from:
    //   base+0, base+32, base+64, ..., base+224 (stride=32)
    // This gives V[K=0..7, D=base] - exactly what MFMA B operand needs!
    //
    // For MFMA B operand layout:
    //   lane_n = lane % 32 (D position 0..31)
    //   lane_k = lane / 32 (K group 0 or 1, for K=0..7 or K=8..15)
    //
    // Each lane reads V[K=0..7 or 8..15, D=lane_n]
    v_and_b32_e32 v7, 31, v0              // lane_n = lane % 32 (D position)
    v_lshrrev_b32_e32 v8, 5, v0           // lane_k = lane / 32 (0 or 1)
    v_lshlrev_b32_e32 v8, 8, v8           // K_group_offset = lane_k * 256 (8 K rows * 32)
    v_add_u32_e32 v7, v8, v7              // D position + K_group_offset
    v_add_u32_e32 v7, LDS_V_OFFSET, v7    // + LDS base
    ds_read_b64_tr_b8 v[64:65], v7         // V[K=0..7 or 8..15, D=lane_n]
    
    s_waitcnt lgkmcnt(0)
    
    // P×V MFMAs for D=0..31 tile, K=0..31 reduction
    // With ds_read_b64_tr_b8, we get V[K=0..7, D=lane_n] for lanes 0-31
    // and V[K=8..15, D=lane_n] for lanes 32-63
    // MFMA 1: P[0..7] × V[K=0..15, D=0..31] → accumulate into O
    v_mfma_f32_32x32x16_fp8_fp8 v[48:63], a[0:1], v[64:65], v[48:63]
    
    // Load V for K=16..31 using transpose read
    // K=16..23: offset 512 from base (16*32)
    // K=24..31: handled by lane_k split
    v_and_b32_e32 v7, 31, v0              // lane_n
    v_lshrrev_b32_e32 v8, 5, v0           // lane_k  
    v_lshlrev_b32_e32 v8, 8, v8           // lane_k * 256 (8 K rows * 32)
    v_add_u32_e32 v7, v8, v7
    v_add_u32_e32 v7, 512, v7             // + K=16 offset (16*32)
    v_add_u32_e32 v7, LDS_V_OFFSET, v7
    ds_read_b64_tr_b8 v[64:65], v7         // V[K=16..23 or 24..31, D=lane_n]
