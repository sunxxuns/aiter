// FP8 QK with XOR swizzle for zero bank conflicts
// Pre-loads ALL Q and K chunks into VGPRs, then chains MFMAs
// 256 threads (4 waves)
// 
// XOR SWIZZLE: addr = (row * 144 + col) XOR (row * 4)
// This eliminates all bank conflicts (64 unique banks for 64 threads)
...
.set XOR_FACTOR, 4
.set Q_LDS, 0
.set K_LDS_A, 4608        // 32 * 144 = 4608
.set K_LDS_B, 9216        // 2 * 4608 = 9216
.set LDS_SIZE, 16384

.text
.globl _ZN5aiter20fwd_fp8_qk_xor_swizzE
.p2align 8
.type _ZN5aiter20fwd_fp8_qk_xor_swizzE,@function

_ZN5aiter20fwd_fp8_qk_xor_swizzE:
    s_mov_b64 exec, -1
    ...
    // LDS offsets with XOR swizzle: addr = (row * 144 + col) XOR (row * 4)
    v_mov_b32_e32 v5, PITCH
    v_mul_lo_u32 v51, v1, v5          // v51 = row * 144
    v_add_u32_e32 v51, v51, v4        // v51 += col
    v_lshlrev_b32_e32 v6, 2, v1       // v6 = row * 4 (XOR factor)
    v_xor_b32_e32 v51, v51, v6        // v51 ^= (row * 4)
    ...
    // MFMA read addresses with XOR swizzle: addr = (row * 144 + k_off) XOR (row * 4)
    v_mov_b32_e32 v1, PITCH
    v_mul_lo_u32 v70, v63, v1         // v70 = row * 144
    v_add_u32_e32 v70, v70, v64       // v70 += k_off
    v_lshlrev_b32_e32 v65, 2, v63     // v65 = row * 4 (XOR factor)
    v_xor_b32_e32 v70, v70, v65       // v70 ^= (row * 4)
    v_add_u32_e32 v70, Q_LDS, v70
    
    // K base address - same swizzle, different base
    // Note: We can't just add an offset because XOR doesn't distribute over addition
    // Need to compute K address separately
    v_mov_b32_e32 v1, PITCH
    v_mul_lo_u32 v71, v63, v1         // v71 = row * 144
    v_add_u32_e32 v71, v71, v64       // v71 += k_off
    v_xor_b32_e32 v71, v71, v65       // v71 ^= (row * 4)
    v_add_u32_e32 v71, K_LDS_A, v71
    ...
    // ========================================================================
    // PRE-LOAD ALL Q CHUNKS (8 x b64 = 64 bytes into v[16:31])
    // Q is reused for all K tiles, so load once
    // ========================================================================
    ...
    // ========================================================================
    // K-LOOP: Pre-load K, then chain MFMAs
    // ========================================================================
    
    // Load K tile 0 to buffer A
    s_mov_b32 s24, 0
    v_add_u32_e32 v54, 0, v50
    buffer_load_dwordx4 v[80:83], v54, s[12:15], 0 offen
    s_waitcnt vmcnt(0)
...
PIPELINE_LOOP:
    ...
    buffer_load_dwordx4 v[84:87], v54, s[12:15], 0 offen
    
    // PRE-LOAD ALL K CHUNKS from buffer A (8 x b64 into v[32:47])
    ...
    // CHAIN ALL 8 MFMAs - no ds_read between them!
    ...
    // Wait for K load to buffer B, write to LDS
    s_waitcnt vmcnt(0)
    ...
    // PRE-LOAD ALL K CHUNKS from buffer B
    ...
    // Wait for K load to buffer A, write to LDS
    s_waitcnt vmcnt(0)
    v_add_u32_e32 v55, K_LDS_A, v51
    ds_write_b128 v55, v[80:83]
    s_waitcnt lgkmcnt(0)
...
.rodata
.p2align 6
.amdhsa_kernel _ZN5aiter20fwd_fp8_qk_xor_swizzE
...
amdhsa.version: [1, 2]
amdhsa.kernels:
  - .name: _ZN5aiter17fwd_fp8_qk_preloadE
    .symbol: _ZN5aiter17fwd_fp8_qk_preloadE.kd
