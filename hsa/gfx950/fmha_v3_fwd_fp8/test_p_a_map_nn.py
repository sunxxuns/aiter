#!/usr/bin/env python3
"""Nearest-neighbor mapping for P->A transpose (post-transpose)."""
import ctypes
import os
import subprocess
import torch
...
def main():
    ...
    P_ref = P[0, 0].detach().cpu()
    out_path = f"{BASE_DIR}/p_a_mapping_nn.csv"
    with open(out_path, "w") as f:
        f.write("lane,pos,row,k,err\n")
        for lane in range(64):
            for pos in range(32):
                ...
                row = idx // 32
                k = idx % 32
                err = float(diff.view(-1)[idx].item())
                f.write(f"{lane},{pos},{row},{k},{err}\n")
