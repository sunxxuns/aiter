...
K_LOOP:
    ...
    // Use ds_read_b64_tr_b8 (transpose read for FP8) to gather K dimension
    // Each thread reads 8 FP8 values along K dimension for one D position
    //
    // MFMA B operand layout:
    // - lane_n = lane_id % 32 (N/D column 0-31)
    // - lane_k = lane_id / 32 (K group 0 or 1)
    // - Each lane provides 8 K values at its D position
    //
    // V LDS layout: V[K, D] row-major, stride_D=1, stride_K=128
    // For lane N, read V[K=0..7, D=N] with stride=128 between K values
    //
    // ds_read_b64_tr_b8: Transpose read for 8-bit data
    // Reads 8 bytes from 8 locations with stride=64 bytes between them
    // So for stride_K=128, we need offset adjustments
    v_and_b32_e32 v7, 31, v0              // lane_n = lane % 32 (D position)
    v_lshrrev_b32_e32 v8, 5, v0           // lane_k = lane / 32 (0 or 1)
    v_lshlrev_b32_e32 v8, 10, v8          // lane_k * 1024 = K_group_offset (8 K rows * 128)
    v_add_u32_e32 v7, LDS_V_OFFSET, v7    // base + D position
    v_add_u32_e32 v7, v8, v7              // + K_group_offset
    ds_read_b64_tr_b8 v[64:65], v7         // V[K=0..7 or 8..15, D=lane%32]
    
    s_waitcnt lgkmcnt(0)
    
    // P×V MFMAs for D=0..31 tile, K=0..31 reduction
    // MFMA: P × V[K=0..15, D=0..31] using transpose-read V data
    v_mfma_f32_32x32x16_fp8_fp8 v[48:63], a[0:1], v[64:65], v[48:63]
    
    // Load V for K=16..31 (second MFMA)
    // Offset by 16 K rows * 128 bytes/row = 2048 bytes
    v_add_u32_e32 v9, 2048, v7            // K=16..31 base
    ds_read_b64_tr_b8 v[64:65], v9
