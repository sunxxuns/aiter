#!/usr/bin/env python3
"""Bit-probe mapping for P->A transpose (post-transpose)."""
import ctypes
import os
import subprocess
import torch
...
def main():
    ...
    k_bits = [run_dump("k", b) for b in range(5)]

    row_val = sum((row_bits[b] << b) for b in range(5))
    k_val = sum((k_bits[b] << b) for b in range(5))

    out_path = f"{BASE_DIR}/p_a_mapping_bits.csv"
