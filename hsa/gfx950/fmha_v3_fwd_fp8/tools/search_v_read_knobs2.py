...
def main() -> None:
    # Focused search grid for the *scaffold-like* mapping.
    # We fix write_mode=2 (bitop3 ttbl=0x7a,c=0x0) because it matches scaffold reads far better.
    write_mode = 2
    # s25 override affects tr8_base; 0xff0 has been a strong candidate.
    s25_overrides = [0xFF0]
    ...
    v4_adds = [0, 16, 32, 48]
    lane_xors = [0, 0x20]
    base_xors = [0x0, 0x20, 0x60, 0x1060, 0x1460]
                    ...
                    for lane_xor in lane_xors:
                        for base_xor in base_xors:
                            ...
                            env = {
                                ...
                                # write_mode lives in bits [9:8] (write_mode<<8)
                            ...
                            if total % 20 == 0:
                                ...
                                print(
                                    ...
                                    f"lane_xor={hex(top.v_read_lane_xor)} base_xor={hex(top.v_read_base_xor)} s25_ov={hex(s25_ov)})"
                                )
